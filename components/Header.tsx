...
export default function Header({ t, currentLocale }: any) {
  ...
  function changeLocale(nextLocale: Locale) { router.push(`/${nextLocale}`); }
      ...
      <div className="mx-auto flex min-h-16 max-w-7xl items-center justify-between gap-4 px-4 py-3 md:px-8">
        ...
        <div className="flex shrink-0 items-center gap-2">
          <select aria-label={t.common.languageLabel} value={currentLocale} onChange={(e) => changeLocale(e.target.value as Locale)} className="h-10 rounded-full border border-white/15 bg-[#211a10] px-3 text-sm text-white outline-none">
            {locales.map((locale) => <option key={locale} value={locale} className="bg-white text-neutral-950">{localeNames[locale]}</option>)}
          </select>
