export default function Reviews({ t }: any) {
  const driveReview = "https://drive.google.com/file/d/17BEsxcLELp7UeyIvJ7-NxrmzbnVRTxVu/preview";
    ...
    <section id="reviews" className="relative overflow-hidden bg-[#15110b] px-5 py-20 text-white md:px-8">
      ...
      <div className="relative mx-auto max-w-7xl">
        ...
        <aside className="mt-8 rounded-[32px] border border-white/10 bg-white/[0.07] p-7 backdrop-blur md:p-8">
          ...
          <p className="mt-5 max-w-4xl leading-7 text-white/70">{t.reviews.cardText}</p>
          <p className="mt-7 max-w-4xl rounded-2xl border border-white/10 bg-black/20 p-4 text-sm leading-6 text-white/55">{t.reviews.disclaimer}</p>
        </aside>
      </div>
    </section>
  );
